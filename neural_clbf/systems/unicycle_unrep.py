...
class UnicycleUNREP(ControlAffineSystem):
    """
    ...
    @property
    def n_dims(self) -> int:
        return UnicycleUNREP.N_DIMS

    @property
    def angle_dims(self) -> List[int]:
        return [UnicycleUNREP.THETA]

    @property
    def n_controls(self) -> int:
        return UnicycleUNREP.N_CONTROLS
    ...
    def state_limits(self) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        upper_limit[UnicycleUNREP.X] = 5.0
        upper_limit[UnicycleUNREP.Y] = 5.0
        upper_limit[UnicycleUNREP.THETA] = np.pi
    ...
    def control_limits(self) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        upper_limit[UnicycleUNREP.V] = 3
        upper_limit[UnicycleUNREP.THETA_DOT] = 3.0 * np.pi
        lower_limit = -1.0 * upper_limit
    ...
    def _f(self, x: torch.Tensor, params: Scenario):
        ...
        f[:, UnicycleUNREP.X, 0] = -1 

        f[:, UnicycleUNREP.Y, 0] = 0

        f[:, UnicycleUNREP.THETA, 0] = 0
    ...
    def _g(self, x: torch.Tensor, params: Scenario):
        ...
        theta = x[:, UnicycleUNREP.THETA]

        # Effect on x
        g[:, UnicycleUNREP.X, UnicycleUNREP.V] = torch.cos(theta)

        # Effect on y
        g[:, UnicycleUNREP.Y, UnicycleUNREP.V] = torch.sin(theta)

        # Effect on theta
        g[:, UnicycleUNREP.THETA, UnicycleUNREP.THETA_DOT] = 1.0
    ...
    def u_nominal(
        ...
        v_scaling = 1.0
        bot_to_origin = -x[:, : UnicycleUNREP.Y + 1].reshape(-1, 1, 2)
        theta = x[:, UnicycleUNREP.THETA]
        bot_facing = torch.stack((torch.cos(theta), torch.sin(theta))).T.unsqueeze(-1)
        u[:, UnicycleUNREP.V] = v_scaling * torch.bmm(bot_to_origin, bot_facing).squeeze()
        ...
        omega_scaling = 5.0
        angle_from_origin_to_bot = torch.atan2(x[:, UnicycleUNREP.Y], x[:, UnicycleUNREP.X])
        ...
        u[:, UnicycleUNREP.THETA_DOT] = -omega_scaling * theta
        u[phi_control_on, UnicycleUNREP.THETA_DOT] = -omega_scaling * phi[phi_control_on]
