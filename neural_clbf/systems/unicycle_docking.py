"""Define a dymamical system for a unicycle model with docking task"""
...
class UnicycleDocking(ControlAffineSystem):
    """
    Represents simplified dynamics for a differential drive boat.
    ...
    @property
    def n_dims(self) -> int:
        return UnicycleDocking.N_DIMS

    @property
    def angle_dims(self) -> List[int]:
        return [UnicycleDocking.THETA]

    @property
    def n_controls(self) -> int:
        return UnicycleDocking.N_CONTROLS
    ...
    def state_limits(self) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        upper_limit[UnicycleDocking.X] = 5.0
        upper_limit[UnicycleDocking.Y] = 5.0
        upper_limit[UnicycleDocking.THETA] = np.pi
    ...
    def control_limits(self) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        upper_limit[UnicycleDocking.V] = 3
        upper_limit[UnicycleDocking.THETA_DOT] = 3.0 * np.pi
        lower_limit = -1.0 * upper_limit
    ...
    def _f(self, x: torch.Tensor, params: Scenario):
        ...
        f[:, UnicycleDocking.X, 0] = 0

        f[:, UnicycleDocking.Y, 0] = 0

        f[:, UnicycleDocking.THETA, 0] = 0
    ...
    def _g(self, x: torch.Tensor, params: Scenario):
        ...
        theta = x[:, UnicycleDocking.THETA]

        # Effect on x
        g[:, UnicycleDocking.X, UnicycleDocking.V] = torch.cos(theta)

        # Effect on y
        g[:, UnicycleDocking.Y, UnicycleDocking.V] = torch.sin(theta)

        # Effect on theta
        g[:, UnicycleDocking.THETA, UnicycleDocking.THETA_DOT] = 1.0
    ...
    def u_nominal(
        ...
        v_scaling = 1.0
        bot_to_origin = -x[:, : UnicycleDocking.Y + 1].reshape(-1, 1, 2)
        theta = x[:, UnicycleDocking.THETA]
        bot_facing = torch.stack((torch.cos(theta), torch.sin(theta))).T.unsqueeze(-1)
        u[:, UnicycleDocking.V] = v_scaling * torch.bmm(bot_to_origin, bot_facing).squeeze()
        ...
        omega_scaling = 5.0
        angle_from_origin_to_bot = torch.atan2(x[:, UnicycleDocking.Y], x[:, UnicycleDocking.X])
        ...
        u[:, UnicycleDocking.THETA_DOT] = -omega_scaling * theta
        u[phi_control_on, UnicycleDocking.THETA_DOT] = -omega_scaling * phi[phi_control_on]
